...
import com.droidtech.novel.utils.ACache;
...
public class CacheManager {
    ...
    public List<BookLists.BookListsBean> getCollectionList() {
        List<BookLists.BookListsBean> list = (ArrayList<BookLists.BookListsBean>) ACache.get(
                BaseApplication.getInstance().getApplication()).getAsObject(getCollectionKey());
        ...
        for (BookLists.BookListsBean bean : list) {
            if (bean != null) {
                if (TextUtils.equals(bean._id, bookListId)) {
                    list.remove(bean);
                    ACache.get(BaseApplication.getInstance().getApplication()).put(getCollectionKey(), (Serializable) list);
                    break;
                }
            }
        }
